...
export function MultiCloudEstate({
  ...
  const environments = useMemo((): EnvironmentNode[] => {
    ...
    for (const cluster of clusters) {
      ...
      // Match project by:
      // 1. Direct foreign-key association (cluster.project_id)
      // 2. Name contains or matches clean name
      // 3. Same provider + same region
      let matchedProject: Project | undefined;

      if (cluster.project_id) {
        matchedProject = projects.find((p) => p.id === cluster.project_id);
      }
